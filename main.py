...
try:
    ...
    Logger.warning("LANG: Arabic libraries (arabic_reshaper, python-bidi) not found. Text may appear disconnected.")

# تسجيل الخط العربي (تأكد من وجود ملف Arial.ttf في نفس المجلد)
FONT_NAME = 'Arial'
# إذا لم يتم العثور على الملف، سيستخدم الخط الافتراضي
if os.path.exists('Arial.ttf'):
    LabelBase.register(name=FONT_NAME, fn_regular='Arial.ttf')
else:
    Logger.warning("FONT: Arial.ttf not found. Using default font.")
    FONT_NAME = 'Roboto' # الخط الافتراضي في Kivy

# قاموس الترجمة
TRANSLATIONS = {
    # تم تعديل اسم التطبيق هنا
    ...
    'switch_lang': {'en': 'عربي', 'ar': 'English'}, # الزر يعرض اللغة المعاكسة
...
class FlashlightApp(FloatLayout):
    ...
    def __init__(self, **kwargs):
        ...
        # زر تشغيل الفلاش (الكبير في المنتصف)
        ...
        mode_layout = BoxLayout(
            ...
            size_hint=(0.9, None), # زيادة العرض لدعم النص العربي
            height=50,
    ...
    def update_ui_texts(self):
        # تحديث نصوص جميع العناصر بناءً على اللغة الحالية
        ...
        # تحديث نص الحالة بناءً على وضع التشغيل الحالي
        status_key = 'tap_on'
    ...
    def update_background(self, *args):
        ...
        with self.canvas.before:
            if self.flashlight_on and self.screen_mode:
                ...
                Color(0.2, 0.2, 0.4, 1) # اللون الأزرق الداكن المفضل
...
class FlashlightMainApp(App):
    def build(self):
        # تم تعديل اسم التطبيق هنا أيضاً
        self.title = 'SALEMFLASH'
        # تعيين الأيقونة للتطبيق (لأنظمة سطح المكتب)
